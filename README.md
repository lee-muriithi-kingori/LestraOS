...
- [x] AI subsystem (7 agentic tools, multi-provider) + in-kernel `pickle` GGUF selftest (KE-28, synced from lestramanika `7ab120a`)
...
- **7 Aug 2026** — 🐛 **lestramanika submodule bumped to `7ab120a` — critical host-only correctness fix.** Upstream lestramanika fixed a bug where `pickle_load_from_file_mmap()` (the default host load path) + a quantized `token_embd.weight` (every real Llama/Mistral/Qwen2 GGUF) produced all-zero logits, causing greedy decoding to always emit `<unk>` and sampling to produce garbage. Root cause: `pickle_attach_mmap()` patches tensor `.data` pointers to raw quantized bytes and detaches `m->io`, but `pickle_fast_state_init()` only dequantized the embedding when `data == NULL` — with mmap, data was non-NULL (raw Q4_K bytes), so the forward pass read raw quant bytes as F32 → garbage hidden states → all-zero logits. Fix: detect the mmap case and dequantize the embedding from raw mmap bytes via `fmemopen()` + `pickle_dequant_stream()` into a fresh F32 buffer. **This bug was entirely host-only** (in `pickle_fast.c` / `pickle_fast.h` / `pickle_host.c` — none of which are vendored into the kernel). The kernel's soft-float path in `pickle.c` was never affected. The submodule bump is for consistency; the kernel sources themselves are byte-identical to `3e671ec` (verified by `sync_lestramanika.sh --check`). Verified: TinyLlama-1.1B-Chat-Q4_K_M now produces coherent output (`"Hello, my name is" → " John Smith. I am a student..."`), selftest still passes, perf unchanged (~11 tok/s decode).
- **6 Aug 2026** — 🧠 **lestramanika wired in as a git submodule + kernel/ai synced to `3e671ec`.** Added [`lestramanika`](https://github.com/lee-muriithi-kingori/lestramanika) (the from-scratch GGUF inference engine, no llama.cpp/ollama/ggml) as a git submodule at `third_party/lestramanika`, pinned to `3e671ec`. New `scripts/sync_lestramanika.sh` vendors the kernel-compatible core (pickle.c / pickle_softfp.c / pickle.h / pickle_demo_gguf.c) into `kernel/ai/` + `kernel/include/lestra/` verbatim — the lestramanika source now uses a conditional `#ifdef PICKLE_KERNEL` include for `pickle.h`, so the sync is a plain copy with no post-processing. Ran the sync: brought the in-kernel pickle up from the KE-28 vendoring to the latest core (adds metadata-only `pickle_load_meta`, on-demand `pickle_dequant_tensor`, raw `pickle_load_tensor_raw`, optional `io->close`, and the conditional include fix). Verified the synced sources compile cleanly under the kernel CFLAGS (`-mno-sse -mno-sse2 -ffreestanding -DPICKLE_KERNEL`); `pickle_selftest()` symbol and declaration unchanged, so the boot-time selftest still prints `next token = 6`. New `docs/LESTRAMANIKA.md` documents the kernel/host split, the submodule, the sync workflow, and the current version table. README updated with a Quick Start submodule step, an AI/lestramanika section, and the architecture diagram now shows `[pickle GGUF selftest]`. The host fast path (AVX-512 VNNI matmul, mmap zero-copy loader, BPE tokenizer, +49% decode) lives exclusively in the lestramanika repo and is built from the submodule.
...
- **5 Aug 2026** — 💾 **KE-24: FAT32 read-write filesystem + VFS integration.** Full write support for the FAT32 driver: cluster allocation via linear FAT scan, FAT entry read-modify-write (write-through), file data writes with cluster chain extension, file creation/deletion, subdirectory creation, directory entry updates. Created `fat32_shim.c` — VFS bridge with fd-based read/write/close (FD range 200..299, 64KB cache per file, write-back on close). Wired FAT32 into VFS as `FS_TYPE_FAT32` with full routing in open/close/read/write/readdir/lseek/mount. Fixed critical bug: FAT mirroring wrote FAT data into data region when NumFATs=1 (data_offset now correctly accounts for NumFATs). Persistence verified: files written in boot N survive reboot in boot N+1, data intact. lestraOS now has persistent storage — the single biggest capability leap since boot.
