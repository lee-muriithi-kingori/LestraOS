...
1. **TIER 2c**: sys_execve + ldso.c argv/envp hardening with LESTRA_ARG_MAX caps
2. **sys_poll revents fix**: Already done (put_user per-element)
3. **Fix pre-existing GP fault** in user_map_page during gui-mode /init ELF loading
4. **KASLR-lite**: Randomize kernel base address
5. **Interrupt-mixed entropy pool** (currently TSC-only, INSECURE)
6. **Driver pacing**: Real RTL8139, AHCI write, PCI enum, USB, framebuffer fonts

### Build Environment
- Toolchain: /home/z/.local/opt/devtools/ (NASM 2.16, QEMU 10.0.11, GRUB 2.12)
- Source env: `source /home/z/.local/opt/devtools/env.sh`
- Build: `export PATH=/home/z/.local/bin:$PATH && export LD_LIBRARY_PATH=/home/z/.local/qemu-prefix/usr/lib/x86_64-linux-gnu:$LD_LIBRARY_PATH && make clean && make all`
- QEMU firmware: symlink qemu-data -> /home/z/.local/opt/devtools/qemu-data
- Boot test (SMEP+SMAP): `qemu-system-x86_64 -cdrom build/lestraos.iso -m 256M -cpu qemu64,+smep,+smap -serial stdio -display none -boot d -net none -L qemu-data`
