...
void ui_boot_splash(void) {
    ...
    write_str_at("        >> lestramk.org - Lightweight OS <<        ", 9, 1, themes[current_theme].prompt);

    /* Status panel */
    ui_panel(11, 2, 10, 76, "Boot Status",
        ...
        "Drivers:    VGA  PS/2 keyboard  Serial COM1  PIT 1000Hz\n"
        "VFS:        in-memory + initrd (max 64 files)\n"
        "Scheduler:  single-task bootstrap (round-robin planned)\n"
        "AI:         agentic subsystem (api-key configurable)\n"
...
void ui_menu_loop(void) {
    ...
    while (1) {
        ...
        if (c == '3') {
            ui_panel(2, 25, 10, 50, "About",
                "Lestra OS\n"
                "lestramk.org\n"
                "(c) 2026 Lee Muriithi Kingori\n"
                "Version 1.0.0-alpha\n"
