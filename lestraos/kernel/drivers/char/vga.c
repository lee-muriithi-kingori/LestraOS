...
/* VGA memory buffer
 * FIX: was 0xFFFFFFFF800B8000 (higher-half alias) but boot.asm only
 * identity-maps the first 8MB via 4 huge 2MB pages. The higher-half
 * alias at 0xFFFFFFFF80000000 would require PDPT[510] to be mapped,
 * which boot.asm does NOT do (it only sets PDPT[0]). Using the
 * identity-mapped physical address 0xB8000 instead. */
