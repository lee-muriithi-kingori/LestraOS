...
`make -C libc` → `build/libc/libc.a` (also built via `powershell .../build-kernel.ps1`).
...
Linked into `kernel.bin` via top-level `Makefile` and `build-kernel.ps1`. Headers used as `#include <string.h>` etc. via `-Ilibc/include`.
