...
static void* sbrk(size_t increment) {
    if (!heap_start) {
        ...
        extern int64_t syscall(uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t);
        void* base = (void*)syscall(9, 0, 0, 0, 0, 0);  /* SYS_BRK with 0 = get current break */
        if (!base) {
...
void exit(int status) {
    extern int64_t syscall(uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t);
    syscall(0, (uint64_t)status, 0, 0, 0, 0);  /* SYS_EXIT */
    while (1);
}
