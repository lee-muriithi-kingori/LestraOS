 ...
 *   - core:      main LestraOS packages (languages, editors, utils)
 *   - websec:    web security tools (nmap, sqlmap, nikto, etc.)
 *   - devtools:  development tools (git, gcc, gdb, valgrind)
 *   - multimedia: media tools (ffmpeg, imagemagick, vlc)
 *   - lestra:    LestraOS-specific packages
 *
 * Commands:
 *   pkg repo list              - list configured repos
 *   pkg repo add <name> <url>  - add a repo
 *   pkg repo remove <name>     - remove a repo
 *   pkg search <query>         - search across all repos
 *   pkg install <name>         - install from any repo
 ...
 *   pkg update                 - refresh repo catalogs
 */
...
int pkg_install(const char* name) {
    ...
    /* HONEST INSTALL FLOW.
     * Previous code printed a fake "Unpacking... / Configuring..." progress
     * bar that did nothing — it just slept and then added the package name
     * to an in-memory list. Users were misled into thinking a real install
     * had happened.
     *
     * Now we are explicit about what "install" means in LestraOS today:
     *   - If the package has a real URL and the network is up, we download
     *     it (already done above) and the bytes land in /var/packages/<name>.
     *   - We register the package as "installed" in the in-memory installed[]
     *     table (lost on reboot, since VFS is in-memory).
     *   - We do NOT unpack, configure, or make the package executable —
     *     LestraOS has no ELF package format, no /var/lib/pkg database,
     *     and no exec-from-VFS path for downloaded binaries yet.
     *
     * The progress bar is replaced with a single status line. No fake
     * "Unpacking/Configuring" theatre.
     */
    if (pkg->url && net_ok) {
        pr_info("  -> downloaded %u bytes to /var/packages/%s\n",
                (unsigned)pkg->size_kb * 1024, pkg->name);
        pr_info("  -> NOTE: package bytes are stored but NOT executed. LestraOS\n");
        pr_info("     has no ELF package runtime yet; you can inspect the file\n");
        pr_info("     with `cat /var/packages/%s`.\n", pkg->name);
    } else if (!pkg->url) {
        pr_info("  -> catalog entry only (no download URL). Registered as\n");
        pr_info("     'installed' in the in-memory package database.\n");
        pr_info("     NOTE: this is a catalog marker, not a real install.\n");
        pr_info("     The package cannot be run from this OS yet.\n");
    } else if (!net_ok) {
        pr_info("  -> network is down; cannot fetch URL. Registered as\n");
        pr_info("     'installed' in the in-memory package database (placeholder).\n");
    }
