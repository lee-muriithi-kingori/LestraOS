/*
 * Lestra OS - Kernel Shell (lsh)
 * Copyright (c) 2026 lestramk.org
 * Full-featured kernel-mode command-line shell
 */
...
/* ----- command history (64-entry ring buffer) ----- */
#define HIST_MAX     64
...
/* Arrow-key hook (PS/2 path) — injects Up/Down sentinels */
...
/* Forward declarations for AI CLI commands */
...
/* AI CLI command implementations - wrappers around ai_chat_with_provider */
void cmd_claude(int argc, char** argv) {
    extern int ai_chat_with_provider(int provider, const char* prompt, char* response, size_t response_size);
    if (argc < 2) { printk("Usage: claude <prompt>\n"); return; }
    extern int ai_provider_from_name(const char* name);
    int provider = ai_provider_from_name("claude");
    char response[4096];
    ai_chat_with_provider(provider, argv[1], response, sizeof(response));
    printk("%s\n", response);
}

void cmd_glm(int argc, char** argv) {
    extern int ai_chat_with_provider(int provider, const char* prompt, char* response, size_t response_size);
    if (argc < 2) { printk("Usage: glm <prompt>\n"); return; }
    extern int ai_provider_from_name(const char* name);
    int provider = ai_provider_from_name("glm");
    char response[4096];
    ai_chat_with_provider(provider, argv[1], response, sizeof(response));
    printk("%s\n", response);
}
...
void cmd_gemini(int argc, char** argv) {
    extern int ai_chat_with_provider(int provider, const char* prompt, char* response, size_t response_size);
    if (argc < 2) { printk("Usage: gemini <prompt>\n"); return; }
    extern int ai_provider_from_name(const char* name);
    int provider = ai_provider_from_name("gemini");
    char response[4096];
    ai_chat_with_provider(provider, argv[1], response, sizeof(response));
    printk("%s\n", response);
}
...
void cmd_openai(int argc, char** argv) {
    extern int ai_chat_with_provider(int provider, const char* prompt, char* response, size_t response_size);
    if (argc < 2) { printk("Usage: openai <prompt>\n"); return; }
    extern int ai_provider_from_name(const char* name);
    int provider = ai_provider_from_name("openai");
    char response[4096];
    ai_chat_with_provider(provider, argv[1], response, sizeof(response));
    printk("%s\n", response);
}
...
void cmd_uai(int argc, char** argv) {
    extern int ai_chat_with_provider(int provider, const char* prompt, char* response, size_t response_size);
    if (argc < 2) { printk("Usage: uai <prompt>\n"); return; }
    extern int ai_provider_from_name(const char* name);
    int provider = ai_provider_from_name("uai");
    char response[4096];
    ai_chat_with_provider(provider, argv[1], response, sizeof(response));
    printk("%s\n", response);
}
